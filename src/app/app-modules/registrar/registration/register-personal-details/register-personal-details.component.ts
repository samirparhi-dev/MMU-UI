...
export class RegisterPersonalDetailsComponent implements OnInit {
  ...
  regexDob = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[1,3-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/;
  ...
  */
  ...
  validateMaritalStatusMaster(revisitData){
    if (revisitData.m_gender.genderID == 3) {
      ...
      this.maritalStatusMaster = this.masterData.maritalStatusMaster.filter((maritalStatus) => {
        if (revisitData.m_gender.genderID == '1' && maritalStatus.maritalStatusID != '5') {
          return maritalStatus
        }
        if (revisitData.m_gender.genderID == '2' && maritalStatus.maritalStatusID != '6') {
          return maritalStatus
        }
      })
    }
  ...
  * Load Editing Data to Form
  ...
  pushEditingDatatoForm(element) {
    ...
    this.personalDetailsForm.patchValue({
      ...
      parentRegID: `${element.benPhoneMaps.length > 0 && element.benPhoneMaps[0].parentBenRegID || null}`,
      parentRelation: `${element.benPhoneMaps.length > 0 && element.benPhoneMaps[0].benRelationshipID || null}`,
      benPhMapID: `${element.benPhoneMaps.length > 0 && element.benPhoneMaps[0].benPhMapID || null}`,
      benRelationshipType: `${element.benPhoneMaps.length > 0 && element.benPhoneMaps[0].benRelationshipType && element.benPhoneMaps[0].benRelationshipType.benRelationshipType || null}`,
      gender: element.m_gender.genderID,
      genderName: element.m_gender.genderName,
      dob: moment(element.dOB).toDate(),
      maritalStatus: element.maritalStatus && element.maritalStatus.maritalStatusID || null,
      maritalStatusName: `${element.maritalStatus && element.maritalStatus.status || null}`,
      ...
      incomeName: element.i_bendemographics && element.i_bendemographics.incomeStatus || null,
    ...
    this.masterData.incomeMaster.forEach(stat => {
      if (element.i_bendemographics.incomeStatus && stat.incomeStatus == element.i_bendemographics.incomeStatus) {
        this.personalDetailsForm.patchValue({
    ...
    if (element.maritalStatus.maritalStatusID == 1 || element.maritalStatus.maritalStatusID == 7) {
    ...
    this._parentBenRegID = `${element.benPhoneMaps.length > 0 && element.benPhoneMaps[0].parentBenRegID || null}`;
  }
  ...
  * get ben image from api
   */
  getBenImage() {
    this.beneficiaryDetailsService.getBeneficiaryImage(this.revisitData.beneficiaryRegID)
      .subscribe(
      data => {
  ...
  */
  ...
  onGenderSelected() {
    ...
    console.log('this.masterData', genderMaster, this.masterData.maritalStatusMaster);

    if (this.personalDetailsForm.value.gender == '3') {
      this.confirmationService.confirm('info', 'You have selected Transgender, please confirm').subscribe((res) => {
        if (!res) {
          this.personalDetailsForm.patchValue({ gender: null, genderName: null });
        } else {
          this.maritalStatusMaster = this.masterData.maritalStatusMaster
        }
      }, (err) => { })
      ...
      this.maritalStatusMaster = this.masterData.maritalStatusMaster.filter((maritalStatus) => {
        ...
        if (this.personalDetailsForm.value.gender == '1' && maritalStatus.maritalStatusID != '5') {
          return maritalStatus
        }
        if (this.personalDetailsForm.value.gender == '2' && maritalStatus.maritalStatusID != '6') {
          return maritalStatus
        }
      })
    }
  ...
  changeLiteracyStatus() {
    ...
    if (literacyStatus != 'Literate') {
      ...
      console.log(this.personalDetailsForm.controls['educationQualification'], 'controls')
    }
 ...
 */
    ...
    if (searchTerm && searchTerm.trim().length === 10) {
      ...
      this.registrarService.identityQuickSearch(searchObject)
        .subscribe(beneficiaryList => {
          if (beneficiaryList && beneficiaryList.length > 0 && beneficiaryList[0].benPhoneMaps.length > 0) {
            console.log('ta d ad a d a', JSON.stringify(beneficiaryList, null, 4));
            this.personalDetailsForm.patchValue({
  ...
  */
  ...
  onAgeEntered() {
    ...
    if (valueEntered) {
      if (valueEntered > this.ageLimit && this.personalDetailsForm.value.ageUnit == 'Years') {
        this.confirmationService.alert(this.currentLanguageSet.alerts.info.ageRestriction, 'info');
        this.personalDetailsForm.patchValue({ age: null });

      } else {
        console.log(moment().subtract(this.personalDetailsForm.value.ageUnit, valueEntered).toDate())
        this.personalDetailsForm.patchValue({ dob: moment().subtract(this.personalDetailsForm.value.ageUnit, valueEntered).toDate() });
      }
  ...
  onAgeUnitEntered() {
    if (this.personalDetailsForm.value.age != null) { this.onAgeEntered(); }
  }
  ...
  */
  dobChangeByCalender(dobval) {
    ...
    if (this.dateForCalendar && (!dobval || dobval.length == 10) && this.personalDetailsForm.controls['dob'].valid) {
      ...
      this.confirmationService.alert(this.currentLanguageSet.alerts.info.invalidData, 'info');
    } else {
      this.personalDetailsForm.patchValue({ age: null });
    }
  ...
  */
  confirmMarriageEligible() {
    if (this.personalDetailsForm.value.age >= this.ageforMarriage && this.personalDetailsForm.value.ageUnit == 'Years') {
  ...
  */
  clearMaritalStatus() {
    if (this.personalDetailsForm.value.maritalStatus != null) {
      this.personalDetailsForm.patchValue({ maritalStatus: null, maritalStatusName: null });
  ...
  */
  ...
  onMaritalStatusChanged() {
    if (this.personalDetailsForm.value.maritalStatus == 1 || this.personalDetailsForm.value.maritalStatus == 7) {
      this.enableMarriageDetails = false;
      this.clearMarriageDetails();
  ...
  */
  checkAgeAtMarriage() {
    if (this.personalDetailsForm.value.ageAtMarriage != null) {
      if (this.personalDetailsForm.value.age == null) {
        this.confirmationService.alert(this.currentLanguageSet.alerts.info.pleaseenterBeneficiaryagefirst, 'info');
        this.personalDetailsForm.patchValue({ ageAtMarriage: null })

      } else if (this.personalDetailsForm.value.ageUnit != 'Years') {
        this.confirmationService.alert(this.currentLanguageSet.alerts.info.marriageAge +' '+this.ageforMarriage +' '+this.currentLanguageSet.alerts.info.years, 'info');
        this.personalDetailsForm.patchValue({ ageAtMarriage: null })
      } else if (this.personalDetailsForm.value.age < this.ageforMarriage) {
        this.confirmationService.alert(this.currentLanguageSet.alerts.info.marriageAge +' '+this.ageforMarriage +' '+this.currentLanguageSet.alerts.info.years, 'info');
        this.personalDetailsForm.patchValue({ ageAtMarriage: null })
      } else if (this.personalDetailsForm.value.ageAtMarriage < this.ageforMarriage) {
        this.confirmationService.alert(this.currentLanguageSet.alerts.info.marriageAge +' '+this.ageforMarriage +' '+this.currentLanguageSet.alerts.info.years, 'info');
        this.personalDetailsForm.patchValue({ ageAtMarriage: null })
      } else if (this.personalDetailsForm.value.age - this.personalDetailsForm.value.ageAtMarriage < 0) {
        this.confirmationService.alert(this.currentLanguageSet.common.marriageatageismorethantheactualage, 'info');
        this.personalDetailsForm.patchValue({ ageAtMarriage: null })
  ...
  getOccupationName() {
    this.masterData.occupationMaster.forEach((occupation) => {
      if (this.personalDetailsForm.value.occupation === occupation.occupationID && this.personalDetailsForm.value.occupation !== 7) {
        this.personalDetailsForm.patchValue({
    ...
    if (!this.personalDetailsForm.value.occupationOther || this.personalDetailsForm.value.occupation == 7) {
      this.personalDetailsForm.patchValue({
  ...
  onEducationQualificationChanged() {
    ...
    qualificationMaster.forEach((element, i) => {
      if (element.educationID === this.personalDetailsForm.value.educationQualification) {
        this.personalDetailsForm.patchValue({
