...
export class OpenModalDirective implements OnInit, DoCheck {
    ...
    openDialog(): void {
        let searchTerm = this.diagnosisListForm.value.provisionalDiagnosis;
        if (searchTerm.length > 2) {
            let dialogRef = this.dialog.open(ProvisionalSearchComponent, {
                ...
                data: {
                    ...
                    diagonasisType: this.currentLanguageSet.DiagnosisDetails.provisionaldiagnosis
                }
            });
            ...
            dialogRef.afterClosed().subscribe(result => {
                if (result) {
                    ...
                    for (let i = len - 1, j = 0; i < len + result.length - 1; i++ , j++) {
                        ...
                        (<FormGroup>formArray.at(i)).controls['provisionalDiagnosis'].setValue(result[j].term);
                        (<FormGroup>formArray.at(i)).controls['provisionalDiagnosis'].disable();
