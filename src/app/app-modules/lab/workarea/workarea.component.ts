  ...
  */
  ...
  checkRange(procedureIndex, componentIndex) {
    ...
    if (component.value.inputValue && (component.value.inputValue < component.value.range_min ||
      ...
      this.confirmationService.alert(this.current_language_set.alerts.info.valueDetails + " " + `${component.value.range_min} to ${component.value.range_max}`);
    }
  }
 ...
 */
  ...
  confirmFormReset() {
    if (this.technicianForm.dirty) {
      this.confirmationService.confirm('info', this.current_language_set.alerts.info.resetDetails)
        .subscribe((res) => {
  ...
  */
  ...
  uploadFile(event, index) {
    ...
    if (!validFormat) {
      this.confirmationService.alert(this.current_language_set.invalidFileExtensionSupportedFileFormats, 'error');
    }
    else {
      if ((this.fileList[0].size / 1000 / 1000) > this.maxFileSize) {
        console.log("File Size" + this.fileList[0].size / 1000 / 1000);
        this.confirmationService.alert(this.current_language_set.fileSizeShouldNotExceed +" "+ this.maxFileSize + " " + this.current_language_set.mb, 'error');
      }
      else if (this.file) {
...
this.confirmationService.alert(this.current_language_set.invalidFileName, 'error');
  }
}
      ...
      if (this.savedFileData && this.savedFileData[procedureID] != undefined) {
        if (this.fileObj[procedureID].length > this.savedFileData[procedureID].length) {
          ...
          if (result && result.length > 0) {
            ...
            this.confirmationService.alert(this.current_language_set.alerts.info.selectNewFile, 'info');
          }
        } else {
          this.confirmationService.alert(this.current_language_set.alerts.info.selectNewFile, 'info');
        }
  ...
  */
  saveFileData(procedureID, fileReq) {
    this.labService.saveFile(fileReq).subscribe((res) => {
      if (res.statusCode == 200) {
        ...
        this.confirmationService.alert(this.current_language_set.alerts.info.successMsg, 'success');
      }
          ...
          for (var fileObjKey in this.fileObj) {
            if (this.savedFileData[fileObjKey]) {
              if (this.savedFileData[fileObjKey].length == this.fileObj[fileObjKey].length) {
                ...
                this.confirmationService.alert(this.current_language_set.alerts.info.uploadSelectedFile);
                break;
              }
            } else {
              this.confirmationService.alert(this.current_language_set.alerts.info.uploadSelectedFile);
  ...
  submitDetails(labCompleted) {
    ...
    // } else {
    if (labCompleted) { option = this.current_language_set.common.submit; } else { option = 'save'; }
    this.confirmationService.confirm('info', this.current_language_set.alerts.info.confirmSubmit + " " + `${option}` + " " + this.current_language_set.alerts.info.labObservation).subscribe((res) => {
      if (res) {
  ...
  canDeactivate(): Observable<boolean> {
    ...
    if (this.technicianForm.dirty)
    return this.confirmationService.confirm(`info`, this.current_language_set.alerts.info.navigateFurtherAlert, 'Yes', 'No');
    else
  ...
  getEcgAbnormalities(){
    this.labService.getEcgAbnormalities().subscribe(res => {
  ...
  checkForEcg(value, procedureName, index){
    if(procedureName.includes('ECG')){
      ...
      this.technicianForm.controls.labForm.value.forEach((item, index) => {
        if(item.procedureName === procedureName){
          // let ecgIndex = index;
          // this.technicianForm.controls.labForm.value[ecgIndex].compListDetails[0].ecgAbnormalities = null;
        item.compListDetails.filter(element => {
...
/* Validation for dupliacte file name */
...
// if (this.fileObj != undefined) {
    ...
    //     result = this.fileObj[procedureID].filter((savedDataName) => {
    //       let arrresult = this.savedFileData[procedureID].filter((uniqueFileName) => {
    //         if (savedDataName.fileName == uniqueFileName.fileName) {
    //           return true
    //         } else {
    //           return false;
    //         }
    ...
    //       this.saveFileData(procedureID, result);
    //     } else {
    //       this.confirmationService.alert("Please select new file to upload", 'info');
    //     }
    //   } else {
    //     if (this.fileObj && this.fileObj[procedureID] && this.fileObj[procedureID].length > 0) {
    //       this.saveFileData(procedureID, this.fileObj[procedureID]);
    //     } else {
    //       this.confirmationService.alert("Please select file to upload", 'info');
    //     }
    //   }
    // } else {
    //   this.confirmationService.alert("Please select file to upload", 'info');
