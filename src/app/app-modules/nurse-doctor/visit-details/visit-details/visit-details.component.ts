    ...
    this.visitCategorySubscription = this.masterdataService.visitDetailMasterData$.subscribe((masterData) => {
      if (masterData) {
        ...
        if(this.beneficiary.ageVal >= 30 && !(this.mode=='view')){
          ...
          this.patientVisitDetailsForm.controls['visitReason'].setValue('New Chief Complaint');
          this.patientVisitDetailsForm.controls['visitCategory'].setValue('NCD screening');
        }
      }
    });
  }
      ...
      .subscribe(value => {
        if (value != null && value.statusCode == 200 && value.data != null) {
          ...
          if (visitCategory == 'NCD care') {
            ...
            visitDetails.visitCode = visitDetails.visitCode;
            this.doctorService.fileIDs = value.data.NCDCareNurseVisitDetail.files;
            this.patientVisitDetailsForm.patchValue(visitDetails);
          }
