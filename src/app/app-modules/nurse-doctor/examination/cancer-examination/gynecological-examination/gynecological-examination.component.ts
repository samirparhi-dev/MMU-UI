...
export class GynecologicalExaminationComponent implements OnInit {
  ...
  annotateImage() {
    this.cameraService.annotate(this.gynaecologicalImage.nativeElement.attributes.src.nodeValue, this.gynecologicalExaminationForm.controls['image'].value,this.currentLanguageSet)
      .subscribe(result => {
        if (result) {
