...
export class GeneralPersonalHistoryComponent implements OnInit {
  ...
  getGeneralHistory(benRegID, visitID) {
    this.generalHistorySubscription = this.doctorService.getGeneralHistoryDetails(benRegID, visitID).subscribe(history => {
      if (history != null && history.statusCode == 200 && history.data != null && history.data.PersonalHistory) {
        this.personalHistoryData = history.data.PersonalHistory;
  ...
  handlePersonalTobaccoHistoryData() {
    let formArray = this.generalPersonalHistoryForm.controls['tobaccoList'] as FormArray;
  ...
  handlePersonalAlcoholHistoryData() {
    let formArray = this.generalPersonalHistoryForm.controls['alcoholList'] as FormArray;
    if (this.personalHistoryData && this.personalHistoryData.alcoholList) {
      ...
      for (let i = 0; i < temp.length; i++) {
        ...
        if (alcoholType.length > 0)
          temp[i].typeOfAlcohol = alcoholType[0];

        temp[i].avgAlcoholConsumption = this.masterData.quantityOfAlcoholIntake.filter(item => {
  ...
  handlePersonalAllergyHistoryData() {
    let formArray = this.generalPersonalHistoryForm.controls['allergicList'] as FormArray;
    if (this.personalHistoryData && this.personalHistoryData.allergicList) {
      ...
      for (let i = 0; i < temp.length; i++) {
        ...
        if (allergyType.length > 0)
          temp[i].allergyType = allergyType[0];

        temp[i].typeOfAllergicReactions = this.masterData.AllergicReactionTypes.filter(item => {
          let flag = false;
        ...
        if (temp[i].otherAllergicReaction)
          temp[i].enableOtherAllergy = true;
  ...
  addTobacco() {
    let tobaccoList = <FormArray>this.generalPersonalHistoryForm.controls['tobaccoList'];
    let temp = tobaccoList.value;
  ...
  addAlcohol() {
    let alcoholList = <FormArray>this.generalPersonalHistoryForm.controls['alcoholList'];
  ...
  addAllergy() {
    ...
    let allergicList = <FormArray>this.generalPersonalHistoryForm.controls['allergicList'];
    if (allergicList.length < this.allergyMasterData.length) {
  ...
  getPreviousTobaccoHistory() {
    let benRegID = localStorage.getItem('beneficiaryRegID');
    this.nurseService.getPreviousTobaccoHistory(benRegID, this.visitCategory)
      .subscribe(res => {
  ...
  getPreviousAlcoholHistory() {
    let benRegID = localStorage.getItem('beneficiaryRegID');
    this.nurseService.getPreviousAlcoholHistory(benRegID, this.visitCategory)
      .subscribe(res => {
        if (res.statusCode == 200 && res.data != null) {
          let title = this.currentLanguageSet.historyData.Alcoholhistory.previousalcoholhistorydetails;
          if (res.data.data.length > 0) {
            this.viewPreviousData(res.data, title);
          } else {
            this.confirmationService.alert(this.currentLanguageSet.historyData.ancHistory.previousHistoryDetails.pastHistoryalert);
  ...
  getPreviousAllergyHistory() {
    let benRegID = localStorage.getItem('beneficiaryRegID');
    this.nurseService.getPreviousAllergyHistory(benRegID, this.visitCategory)
      .subscribe(res => {
        if (res.statusCode == 200 && res.data != null) {
  ...
  canEnableOtherAllergy(allergy) {
    ...
    allergicList.forEach(item => {
      if (item.allergicReactionTypeID == 11)
        flag = true;
    })
    if (!flag)
      allergy.patchValue({ otherTypeOfAllergicReaction: null });
  ...
  get alcoholIntakeStatus() {
    return this.generalPersonalHistoryForm.controls['alcoholIntakeStatus'].value;
  }
  ...
  validateDuration(formGroup: FormGroup, event?: Event) {
    ...
    console.log(duration, durationUnit, this.beneficiary, this.beneficiary.ageVal);
  ...
  checkTobaccoValidity(tobaccoForm) {
    ...
    if (temp.tobaccoUseType && temp.number && temp.duration && temp.durationUnit) {
      return false;
    } else {
      return true;
    }
  ...
  checkAlcoholValidity(alcoholForm) {
    ...
    if (temp.typeOfAlcohol && temp.alcoholIntakeFrequency && temp.avgAlcoholConsumption && temp.duration && temp.durationUnit) {
      return false;
    } else {
      return true;
    }
  ...
  checkAllergyValidity(allergyForm) {
    ...
    if (temp.allergyType && temp.typeOfAllergicReactions && temp.snomedTerm && temp.snomedCode) {
      return false;
    } else {
      return true;
    }
