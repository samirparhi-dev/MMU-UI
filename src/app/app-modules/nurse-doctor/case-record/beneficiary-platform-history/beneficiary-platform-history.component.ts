...
export class BeneficiaryPlatformHistoryComponent implements OnInit {
  ...
  constructor(private doctorService: DoctorService,
    ...
    private dialog: MdDialog, private httpServiceService:HttpServiceService) { }
  ...
  getEachVisitData(){
    this.historyOfMMU.forEach((item, i) => {
      if(item.visitCode){
        let reqObj = {
        ...
        this.doctorService.getMMUCasesheetData(reqObj).subscribe(res => {
  ...
  previousMMUHistoryPageChanged(event): void {
    ...
    this.previousMMUHistoryPagedList = this.historyOfMMU.slice(startItem, endItem);
