import { Component, OnInit, Input, OnChanges, ViewChild, OnDestroy, ChangeDetectorRef, ViewEncapsulation } from '@angular/core';
    ...
    if (event.isUserInput) {
      if (this.checkNotIssued(option.itemID)) {
        ...
        if (option.quantityInHand == 0) {
          this.confirmationService.confirm('info ' + typeOfDrug, this.current_language_set.stockNotAvailableWouldYouPrescribe + ' ' + option.itemName + ' ' + option.strength + option.unitOfMeasurement)
          .subscribe(res => {
            if (!res) {
              this.tempDrugName = null;
