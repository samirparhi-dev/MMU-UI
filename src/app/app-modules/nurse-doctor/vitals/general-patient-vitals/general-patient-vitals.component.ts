...
export class GeneralPatientVitalsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.nurseService.ncdTemp$.subscribe(response => (response == undefined ? this.ncdTemperature = false : this.ncdTemperature = response));
  ...
  ngOnChanges() {
    ...
    if (this.visitCategory == 'ANC' || this.visitCategory == 'General OPD (QC)') {
  ...
  checkNurseRequirements(medicalForm) {
    ...
    if (this.visitCategory == 'NCD screening') {
      ...
      if (vitalsForm.controls['waistCircumference_cm'].errors) {
       required.push(this.currentLanguageSet.vitalsDetails.vitalsCancerscreening_QC.waistCircumference);
      }
      if (vitalsForm.controls['temperature'].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.vitalsDataANC_OPD_NCD_PNC.temperature);
      }
      if (vitalsForm.controls['pulseRate'].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.vitalsDataANC_OPD_NCD_PNC.pulseRate);
      }
      if (vitalsForm.controls['systolicBP_1stReading'].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.vitalsDataANC_OPD_NCD_PNC.systolicBP);
      }
      if (vitalsForm.controls['diastolicBP_1stReading'].errors) {
        required.push(this.currentLanguageSet.vitalsDetails.vitalsDataANC_OPD_NCD_PNC.diastolicBP);
      }
  ...
  updateGeneralVitals(patientVitalsForm) {
    if (this.checkNurseRequirements(this.patientVitalsForm)) {
      this.doctorService.updateGeneralVitals(patientVitalsForm, this.visitCategory)
        .subscribe((res: any) => {
  ...
  getGeneralVitalsData() {
    this.generalVitalsDataSubscription = this.doctorService.getGenericVitals({
      benRegID: localStorage.getItem('beneficiaryRegID'),
      benVisitID: localStorage.getItem('visitID')
    }).subscribe(vitalsData => {
      if (vitalsData) {
        let temp = Object.assign({}, vitalsData.benAnthropometryDetail, vitalsData.benPhysicalVitalDetail);
        ...
        if (this.patientVitalsForm.controls['hipCircumference_cm'].value && this.patientVitalsForm.controls['hipCircumference_cm'].value != null && this.visitCategory == 'General OPD') {
          this.checkHip(this.patientVitalsForm.controls['hipCircumference_cm'].value);
        }
        if (this.patientVitalsForm.controls['waistHipRatio'].value && this.patientVitalsForm.controls['waistHipRatio'].value != null && this.visitCategory == 'General OPD') {
          this.hipWaistRatio();
        }
        this.calculateBMI();
  ...
  calculateBMI() {
    ...
    (this.benGenderAndAge.genderName).toLowerCase() =="female")) {
      this.nurseService.calculateBmiStatus({
  ...
  hipWaistRatio() {
    let waistHipRatio;
    if (this.hipCircumference_cm && this.waistCircumference_cm && this.hipCircumference_cm != null && this.waistCircumference_cm != null) {
      waistHipRatio = (this.waistCircumference_cm / this.hipCircumference_cm).toFixed(2);
  ...
  checkMidUpperArmCircumference(midUpperArmCircumference_MUAC_cm) {
    if (this.midUpperArmCircumference_MUAC_cm <= 6 || this.midUpperArmCircumference_MUAC_cm >= 30) {
      this.confirmationService.alert(this.currentLanguageSet.alerts.info.recheckValue);
    }
  }
  ...
  checkSystolic(systolicBP) {
    ...
    if(systolicBP !=null)
      this.idrsscore.setSystolicBp(systolicBP);
    else
    this.idrsscore.setSystolicBp(0);
  ...
  checkDiastolic(diastolicBP) {
    ...
    if(diastolicBP != null)
      this.idrsscore.setDiastolicBp(diastolicBP);
    else
      this.idrsscore.setDiastolicBp(0);
  ...
  get midUpperArmCircumference_MUAC_cm() {
    return this.patientVitalsForm.controls['midUpperArmCircumference_MUAC_cm'].value;
  }
