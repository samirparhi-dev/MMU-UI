...
export class IdrsComponent implements OnInit {
  ...
  /* Vitals and history */
  idrsWaistScore() {
    this.idrsWaistSubscription = this.idrsScoreService.IDRSWaistScore$.subscribe((response) => {
      response === undefined
  ...
  idrsFamilyHistoryScore() {
    this.IdrsFamilyScoreSubscription = this.idrsScoreService.IDRSFamilyScore$.subscribe((response) => {
      response == undefined
  ...
  idrsPhysicalScoreActivity() {
    this.idrsPhysicalScoreSubscription = this.idrsScoreService.IDRSPhysicalActivityScore$.subscribe((response) => {
      response == undefined
  ...
  uncheckedDiseases() {
    this.uncheckedDiseaseSubscription = this.idrsScoreService.uncheckedDiseases$.subscribe((response) => {
  ...
  visitDiseases() {
    this.visitDiseaseSubscription = this.idrsScoreService.visitDiseases$.subscribe((response) => {
  ...
  idrsFlagScore() {
    this.IdrsScoreFlagSubscription = this.idrsScoreService.IDRSScoreFlagCheck$.subscribe((response) => {
  ...
  checkQuestionsToAddInSuspect(disease) {
    for (let question of this.questions1) {
      if (
        ...
        for (let value of this.diseases) {
          if (value.disease.indexOf("Diabetes") > -1)
            value.confirmed = false;
        }
        break;
      }
  ...
  systolicBPObs(response) {
    ...
    if (response !== null && response >= 140) {
      ...
      if (this.suspect.length > 0)
        this.removeSuspectedHyper("Hypertension");
    }
  }
  ...
  ngOnDestroy() {
    ...
    if (this.idrsWaistSubscription) 
      this.idrsWaistSubscription.unsubscribe();
  ...
  removeDiagnosisConfirmedDisease(disease, tokenTodeleteDisease) {
      ...
      (res: any) => {
        if (res.statusCode == 200 && res.data != null) {
          ...
          if (
            ...
            if (this.age >= 30) {
              for (let disease of this.diseases) {
                if (disease.disease.indexOf("Diabetes") > -1 && this.isDiabetic) {
                  disease.flag = false;
                  disease.disabled = true;
                }
                else if (disease.disease.indexOf("Diabetes") > -1 && this.idrsScore + this.idrsScoreWaist + this.idrsScoreFamily + this.IRDSscorePhysicalActivity < 60) {
                  disease.flag = false;
                } else if (disease.disease.indexOf("Vision Screening") > -1 && this.isVision) {
                  disease.flag = false;
                  if (this.isVision) disease.disabled = true;
                } else if (disease.disease.indexOf("Epilepsy") > -1 && this.isEpilepsy) {
                  disease.flag = false;
            ...
            if (
              ...
              if (this.confirmDiseaseArray !== undefined && this.confirmDiseaseArray.length === 0 && 
              ...
              if (this.isVision) this.addToconfirmDiseaseArray("Vision Screening");
  ...
  updateFormWithSuspectConfirmQuestionArray() {
    if (
      ...
      this.idrsScreeningForm.patchValue({ confirmArray: this.confirmDiseaseArray });
      this.updateDiabetesQuestionValue();
      console.log("diseaseList", this.diseases);
    ...
    this.IDRSDetailsSubscription = this.doctorService
      ...
      .subscribe(value => {
        if (value !== null && value.statusCode === 200 && value.data !== null) {
          ...
          for (var j = 0; j < IdrsArray.length; j++) {
          ...
          if (this.questions != undefined && this.questions.length > 0) {
            ...
            if (this.age >= 30) {
              ...
              for (let disease of this.diseases) {
                ...
                if (disease.disease.indexOf("Diabetes") > -1 && this.idrsScore + this.idrsScoreWaist + this.idrsScoreFamily + this.IRDSscorePhysicalActivity < 60) {
                  disease.flag = false;
                }
                if (
                  disease !== undefined &&
  ...
  radioChange(q, value, d) {
    this.IDRSChanged.emit(false);
    for (const question of this.questions1) {
      if (question.idrsQuestionID === q.idrsQuestionID)
    ...
    if (value == "yes") this.addToSuspected(q.diseaseQuestionType);
      ...
      if (this.suspect.length > 0)
        this.removeSuspected(q.diseaseQuestionType);
    }
    ...
    if (
      ...
      for (let questionValue of this.questions1) {
        if (questionValue.answer != null)
          this.arr.push(questionValue);
      }
    }
    this.required = [];
    ...
    for (let disease of this.diseases) {
      if (disease.disease === d) {
        ...
        for (let question of this.questions1) {
          if (
            question.diseaseQuestionType === d &&
            question.answer === null
          ) {
            Rflag = true;
            break;
  ...
  addToSuspectedHyper(val) {
    ...
    if (!suspectflag && !confirmflag) {
      ...
      if (val === "Hypertension") {
        if (!(this.suspect.length == 1 && this.suspect.includes("Hypertension"))) {
          this.IDRSChanged.emit(false);
          this.systolicChange = true;
        }
        else if (this.systolicChange)
          this.IDRSChanged.emit(false);
      }
      // if (val == "Diabetes")
      //   this.idrsScoreService.setDiabetesSelected();
  ...
  removeSuspectedHyper(val) {
    let flag = false;
    for (let question of this.questions1) {
      if (
        question.diseaseQuestionType === val &&
        question.answer === "yes"
      )
        flag = true;
    }
      ...
      if (this.suspect.length == 0) {
        ...
        if (
          !(this.suspect.includes("Vision Screening") ||
            this.suspect.includes("Epilepsy") || this.suspect.includes("Asthma") ||
            this.suspect.includes("Tuberculosis Screening") || this.suspect.includes("Malaria Screening"))
        ) {
          this.idrsScoreService.clearSuspectedArrayFlag();
        }
        else {
  ...
  removeConfirmDiseaseArray(val) {
    ...
    if (!flag) {
      for (let i = 0; i < this.confirmDiseaseArray.length; i++) {
        if (this.confirmDiseaseArray[i] === val) this.confirmDiseaseArray.splice(i, 1);
      }
